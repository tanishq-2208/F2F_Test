...
class _MyOrdersScreenState extends State<MyOrdersScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadOrders() async {
    try {
      ...
      // Add the required status parameter to getCustomerOrders
  ...
  Future<void> _loadRandomDeliveredOrders() async {
    try {
      // Get all delivered orders that haven't been rated yet
      ...
      final querySnapshot = await FirebaseFirestore.instance
          ...
          .where('isRated', isEqualTo: false) // Only get unrated orders
          .get();
  ...
  Widget _buildSwiggyStyleRatingCard(Map<String, dynamic> order) {
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          // Order header with green background
          Container(
            padding: const EdgeInsets.all(16),
  ...
  Future<void> _rateFarmer(Map<String, dynamic> order, {int rating = 0}) async {
    ...
    // If rating is provided directly, use it, otherwise navigate to rating screen
    if (rating > 0) {
      try {
    ...
    // Otherwise use the detailed rating screen
    ...
    if (result == true) {
      // Refresh orders list to update rating status
      _loadOrders();
      _loadRandomDeliveredOrders();
    }
  }
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                if (order['status'] == 'Delivered' && !(order['isRated'] ?? false))
                  TextButton.icon(
